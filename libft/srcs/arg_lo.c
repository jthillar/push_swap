...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/06/22 10:08:01 by jthillar          #+#    #+#             */
/*   Updated: 2017/07/20 16:37:54 by jthillar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

char	*arg_lo_prev(t_print p, unsigned long n, char *str, char *fill)
{
	int		size;
	char	*tmp;

	if (p.point == 1)
		size = p.prec - ft_strlen(ft_itoabaselong(n, 8));
	else
		size = p.weigth_n - ft_strlen(ft_itoabaselong(n, 8));
...
char	*arg_lo_prev2(t_print p, char *str, char *fill, unsigned long n)
{
	char *tmp;
...
int		all_zero_lo(t_print p, t_str *list, unsigned long n)
{
...
void	arg_lo(t_print p, unsigned long n, t_str *list)
{
	char	*str;
	char	*fill;

	fill = NULL;
	if (all_zero_lo(p, list, n) == 0)
		return ;
	str = ft_itoabaselong(n, 8);
	if (n == 0 && p.point == 1 && p.prec == 0 && p.htag == 0)
		str[0] = '\0';
	if ((p.point == 1 && p.prec > ft_strlen(str)) ||
	(p.weigth_zero == 1 && p.prec < p.weigth_n))
	{
		str = arg_lo_prev(p, n, str, fill);
	...
	(p.weigth_n > ft_strlen(ft_itoabaselong(n, 8))))
		str = arg_lo_prev2(p, str, fill, n);
