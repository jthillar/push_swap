...
/*   Updated: 2017/07/20 14:01:46 by jthillar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static char	*arg_d_prev_min(size_t size, char *fill, int n, t_print p)
{
	char *tmp;
...
static char	*arg_d_prev(t_print p, int n, char *str, char *fill)
{
	size_t	size;
	char	*tmp;
	...
	if (n >= 0)
	{
		...
		tmp = arg_d_prev_min(size, fill, n, p);
	tmp = fill_by_left(tmp, p, '0');
	return (tmp_to_str(tmp, str, fill));
}

static char	*arg_d_prev2(t_print p, char *str, char *fill, int n)
{
	char *tmp;

	if (!(fill = ft_strnew(p.weigth_n - ft_intlen(n))))
		return (NULL);
...
void		arg_d(t_print p, int n, t_str *list)
{
	char	*str;
	char	*fill;
