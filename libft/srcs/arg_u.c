...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/06/21 18:57:57 by jthillar          #+#    #+#             */
/*   Updated: 2017/07/20 16:43:03 by jthillar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static char	*arg_u_prev(t_print p, unsigned int n, char *str, char *fill)
{
	int		size;
	char	*tmp;
...
static char	*arg_u_prev2(t_print p, char *str, char *fill, unsigned int n)
{
	char *tmp;
...
static int	all_zero_u(t_print p, t_str *list, unsigned int n)
{
...
void		arg_u(t_print p, unsigned int n, t_str *list)
{
	char	*str;
	char	*fill;

	fill = NULL;
	if (all_zero_u(p, list, n) == 0)
		return ;
	...
	&& (p.weigth_n > ft_strlen(ft_itoabase(n, 10))))
		str = arg_u_prev2(p, str, fill, n);
	add_list_del_str(list, str);
}
