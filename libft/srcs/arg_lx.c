...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/06/22 09:54:01 by jthillar          #+#    #+#             */
/*   Updated: 2017/07/20 16:38:45 by jthillar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static char	*arg_lx_prev(t_print p, unsigned long n, char *str, char *fill)
{
	int		size;
	int		diese;
	char	*tmp;
	...
	if (p.htag == 1 && p.point == 0)
		diese = -2;
	if (p.point == 1)
		size = p.prec - ft_strlen(ft_itoabaselong(n, 16)) + diese;
	else
		size = p.weigth_n - ft_strlen(ft_itoabaselong(n, 16)) + diese;
...
static char	*arg_lx_prev2(t_print p, char *str, char *fill, long n)
{
	char *tmp;
...
static int	all_zero_lx(t_print p, t_str *list, unsigned long n)
{
...
void		arg_lx(t_print p, unsigned long n, t_str *list)
{
	char	*str;
	char	*fill;

	fill = NULL;
	if (all_zero_lx(p, list, n) == 0)
		return ;
